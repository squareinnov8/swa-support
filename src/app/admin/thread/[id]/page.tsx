...
export default async function ThreadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Fetch messages - newest first so admin sees most recent content at top
  ...
  const latestDraftMessage = draftMessages[0]; // Most recent draft (array is newest-first)

  // Get the customer email from the first inbound message (array is newest-first, so find the last one)
  const firstInboundMessage = [...(messages || [])].reverse().find(m => m.direction === "inbound");
