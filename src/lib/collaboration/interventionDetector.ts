 ...
 * by looking for the message in our messages table with sent_by_agent flag
 */
export async function wasMessageGeneratedByAgent(
  supabase: typeof import("@/lib/db").supabase,
  gmailMessageId: string
): Promise<boolean> {
  // Check if we have a message record with this Gmail ID that was sent by the agent
  // Agent-sent messages have sent_by_agent: true in channel_metadata
  const { data } = await supabase
    .from("messages")
    .select("id, channel_metadata")
    .eq("channel_metadata->>gmail_message_id", gmailMessageId)
    .maybeSingle();

  if (!data) return false;

  // Check if this message was sent by the agent (not just synced from Gmail)
  const metadata = data.channel_metadata as Record<string, unknown> | null;
  return metadata?.sent_by_agent === true;
}
